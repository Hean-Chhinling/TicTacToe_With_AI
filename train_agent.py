...
def get_state_id(state, state_id):
    state_rep = state_to_str(state)
    if state_rep not in state_id:
        state_id[state_rep] = len(state_id)
...
def play_episodes(n_episodes, max_epsilon=1.0, min_epsilon=0.05, decay_rate=0.0001, gamma=0.9, learn=True, enemy=None):
    ...
    for episode in tqdm(range(n_episodes)):
        ...
        while not is_done:
            ...
            state_representation = state_to_str(state)
            new_state_representation = get_state_id(new_state, state_to_id)
            ...
            state_representation = get_state_id(state, state_to_id)
...
# Train the agent
rewards, epsilon_history = play_episodes(n_episodes=50_000_000, max_epsilon=1.0, min_epsilon=0.05,
...
def main():
    ...
    first_move_counts = {i: first_moves.count(i) for i in range(9)}
    print("First move counts:", first_move_counts)
